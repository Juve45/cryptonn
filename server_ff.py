...
def back_prop(x, y, parameters, cache, conn):
	...
	server_send(conn, dW2)
	server_send(conn, db2)
...
def train(Y, parameters, alpha = 0.01, epoch = 10):
	acc =[]
	losss =[]
	conn = create_socket()
	...
	for j in range(epoch):
		...
		for i in range(len(x)):
			...
			acc_1 += loss[0]
			loss_1 += loss[1]
			gradients = back_prop(x[i], y[i], parameters, cache, conn)
...
acc, losss, parameters = train(y, parameters, 0.005, 30)
